...
function App() {
  ...
  useEffect( function () {   
    async function getFact() {
      ...
      const { data: facts, error } = await query.order("voteIntresting", {ascending: false})
      .limit(100);
...
function Header({showForm,setShowForm}) {
  return (
    ...
    {/* 3. Update the state (N.B: using the `set`) */}
...
function NewFactForm({setFacts, setShowForm}) {
  ...
  async function handleSubmit(e) {
    ...
    if (text && isValidHttpUrl(source) && category && textLength <= 200) console.log("Valid form")
  ...
  <form className="fact-form" onSubmit={handleSubmit}>
    <input value={text} type="text" placeholder="Share a fact with World..."  
    ...
    <select value={category} name="" id="" onChange={(e) => setCategory(e.target.value)} disabled={isUploading}>
      <option value="" >Choose Category:</option>
      {CATEGORIES.map((cat) => <option key={cat.name} value={cat.name}>{cat.name.toUpperCase()}
      </option>)}
    </select>
...
function FactList({facts}) {
  if (facts.length === 0) {
    return (
      <p className="message">No fact for this category yet. Create the first one!✌</p>
    )
  }
  return (
...
function Fact({fact}) {
  async function handleVote() {
    ...
    .from("facts")
    .update({voteIntresting: fact.voteIntresting +1})
    .eq("id" === fact.id)
    .select()
  ...
  return (
    <li className="fact">
      <p>
        {fact.text}
        <a className="source" href={fact.source} target="blank">(Source)</a>
      </p>
      <span className="tag" style={{backgroundColor: CATEGORIES.find((cat) => 
        cat.name === fact.category
        )?.color}}>{fact.category}</span>
      <div className="vote-buttons">
        <button onClick={handleVote}>👍:{fact.voteIntresting}</button>
